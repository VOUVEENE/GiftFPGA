...
try:
    import gift_adj_cpp
    CPP_AVAILABLE = True
    logging.info("C++邻接矩阵构建模块已加载")
except ImportError:
    CPP_AVAILABLE = False
    logging.warning("C++扩展未找到，将使用Python实现")
...
class GiFtFPGAPlacer:
    ...
    def build_adjacency_matrix(self):
        """构建邻接矩阵 - 优先使用C++"""
        if CPP_AVAILABLE:
            self.build_adjacency_matrix_cpp()
        else:
            self.build_adjacency_matrix_python()
    
    def build_adjacency_matrix_cpp(self):
        """使用C++构建邻接矩阵"""
        logger.info("使用C++构建邻接矩阵...")
        ...
        logger.info(f"C++邻接矩阵构建完成，耗时: {build_time:.3f}s, "
                   f"非零元素: {self.adjacency_matrix.nnz:,}, "
                   f"密度: {density:.4f}%")
    
    def build_adjacency_matrix_python(self):
        """Python回退实现"""
        logger.info("使用Python构建邻接矩阵...")
        start_time = time.time()
        ...
        logger.info(f"Python邻接矩阵构建完成，耗时: {build_time:.3f}s, "
                   f"非零元素: {self.adjacency_matrix.nnz:,}, "
                   f"密度: {density:.4f}%")
    ...
    def initialize_positions(self):
        ...
        # 使用DREAMPlace风格的初始位置生成
        fixed_positions = initial_positions[placedb.num_movable_nodes:]
        random_initial = self.generate_initial_locations(
            fixed_positions, placedb.num_movable_nodes, self.scale)
    ...
    def apply_placement_constraints(self, positions):
        ...
        for i in range(placedb.num_movable_nodes):
            ...
            if self.enable_boundary_constraints:
                center_x = max(placedb.xl + half_width, 
                              min(center_x, placedb.xh - half_width))
                center_y = max(placedb.yl + half_height,
                              min(center_y, placedb.yh - half_height))
    ...
    def optimize_placement(self):
        ...
        # 应用GiFt算法
        self.optimized_positions = self.apply_gift_filters(self.initial_positions)
